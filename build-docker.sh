...
IMAGE_NAME=darts-caller-pipewire
...
do
    case "${flag}" in
        v) DCALLER_VERSION=${OPTARG};;
        u) DOCKER_USER=${OPTARG};;
    esac
done

# Check if version is set
if [ -z "$DCALLER_VERSION" ]; then
...
docker buildx build --platform $PLATFORMS --build-arg="REF=${DCALLER_VERSION}" -t $IMAGE_NAME:latest -t $IMAGE_NAME:$DCALLER_VERSION --push .
